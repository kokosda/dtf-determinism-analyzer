...
public static class Activities
{
    ...
    public static async Task<string> ReadConfigFileActivity([ActivityTrigger] string fileName, ILogger logger)
    {
        try
        {
            ...
            else
            {
                ...
                var sampleConfig = $"# Sample config created at {DateTime.Now:yyyy-MM-dd HH:mm:ss}";
    ...
    public static Task<string> GetEnvironmentVariableActivity([ActivityTrigger] string variableName, ILogger logger)
    {
        ...
        var value = Environment.GetEnvironmentVariable(variableName) ?? $"Not found: {variableName}";
    ...
    public static async Task<string> FetchDataActivity([ActivityTrigger] string url, ILogger logger)
    {
        try
        {
            ...
            var mockResponse = $"Mock data from {url} at {DateTime.Now:yyyy-MM-dd HH:mm:ss}";
    ...
    public static async Task<string> ProcessChunkActivity([ActivityTrigger] object input, ILogger logger)
    {
        ...
        var random = new Random();
        var processingTime = random.Next(500, 2000);
        await Task.Delay(processingTime);
        
        var result = $"Chunk processed: {input} (took {processingTime}ms)";
    ...
    {
        var combined = string.Join(" | ", chunkResults);
        var result = $"Final: [{combined}] - Completed at {DateTime.Now:HH:mm:ss}";
