...
namespace DtfDeterminismAnalyzer.CodeFixes
{
    ...
    public class Dfa0007ThreadSleepCodeFix : CodeFixProvider
    {
            ...
            InvocationExpressionSyntax invocation)
        {
            ...
            ITypeSymbol? argumentType = semanticModel.GetTypeInfo(argument.Expression, context.CancellationToken).Type;
            ...
            {
                title = argumentType?.Name == "TimeSpan"
                    ? "Replace Thread.Sleep(TimeSpan) with await context.CreateTimer()"
                    : "Replace Thread.Sleep() with await context.CreateTimer()";
            }
            ...
            InvocationExpressionSyntax invocation, CancellationToken cancellationToken)
        {
            ...
            InvocationExpressionSyntax createTimerCall = SyntaxFactory.InvocationExpression(
                SyntaxFactory.MemberAccessExpression(
                    SyntaxKind.SimpleMemberAccessExpression,
                    SyntaxFactory.IdentifierName(orchestratorContextParameter),
            ...
            AwaitExpressionSyntax awaitExpression = SyntaxFactory.AwaitExpression(createTimerCall);
        ...
        {
            MethodDeclarationSyntax? method = originalNode.FirstAncestorOrSelf<MethodDeclarationSyntax>();
            if (method == null)
            {
                return root;
            }
            ...
            SyntaxTokenList newModifiers = method.Modifiers.Add(asyncModifier);
